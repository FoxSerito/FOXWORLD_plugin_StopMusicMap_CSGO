...
{
	name = "[FOXWORLD] Stop Map Music",
	author = "FoxSerito",
	description = "Allows clients to stop sounds played by the map",
...
public OnPluginStart()
{
	...
	CreateTimer(3.0, Post_Start, _, TIMER_REPEAT);
}
...
public void Event_RoundEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	for(new iPlFox=1;iPlFox<=MaxClients;iPlFox++){
		for (new i = 0; i < g_iNumSounds; i++)
		{
			...
			if (entity != INVALID_ENT_REFERENCE)
			{
				...
				Client_StopSound(iPlFox, entity, SNDCHAN_STATIC, sSound);
			}
		}
	}
}
...
public Action:Command_StopMusic(client, args)
{
	...
	if(disabled[client]){
		disabled[client] = false;
		PrintToChat(client, "[MAPMUSIC] Музыка на карте разрешена.");
		return Plugin_Handled;
	}

	g_fCmdTime[client] = GetGameTime() + 5.0;

	PrintToChat(client, "[MAPMUSIC] Останавливаем музыку на карте...");
